...
# CP4c _end-leak FIX (Task #266): sheets that ONLY the FRONT-END (-DP6_FRONTEND_LOGOS)
# boot stages. Their probe rows MUST be flag-gated in p6_sheet_probes.inc so the
# DEFAULT (GHZ) shipping build does NOT compile them into the unconditional const
# p6SheetProbes[] (each row = 24 B of .rodata; 3 Logos rows = 72 B pushed _end from
# 0x060B6BA0 to 0x060B6BF0, 16 B under the P6_HW_ANIMPAK 0x060B6C00 ceiling = a
# #228 boot-trap landmine). The GHZ build never stages LOGOS.SHT (its slot-9 probes
# would also FetchRect-fail at runtime), so gating is correct on both counts. Keyed
# by the .SHT output name so the model/.SHT asset emission is unchanged. */
FRONTEND_ONLY_SHEETS = {"LOGOS.SHT"}
...
def main():
    ...
    with open(ipath, "w") as f:
        ...
        # CP4c _end-leak FIX (Task #266): split the rows into DEFAULT (every GHZ
        # shipping sheet) and FRONT-END-ONLY (LOGOS.SHT etc., staged only under
        # -DP6_FRONTEND_LOGOS). The front-end rows are wrapped in
        # #if defined(P6_FRONTEND_LOGOS) so the DEFAULT build's const table is the
        # 27-row (byte-identical pre-Logos) array -- _end stays at 0x060B6BA0, well
        # clear of the 0x060B6C00 ANIMPAK ceiling. The slot index si is the staging
        # order (0..N) and is preserved in both branches.
        base_rows = []
        fe_rows = []
        for si, m in enumerate(model["sheets"]):
            dst = fe_rows if m["file"] in FRONTEND_ONLY_SHEETS else base_rows
            for p in m["probes"]:
                dst.append("    { %d, %d, %d, %d, %d, 0x%08Xu }"
                           % (si, p["sx"], p["sy"], p["w"], p["h"],
                              int(p["hash"], 16)))
        if fe_rows:
            f.write("#if defined(P6_FRONTEND_LOGOS)\n")
            f.write("#define P6_SHEET_PROBE_COUNT %d\n" % (len(base_rows) + len(fe_rows)))
            f.write("#else\n")
            f.write("#define P6_SHEET_PROBE_COUNT %d\n" % len(base_rows))
            f.write("#endif\n")
        else:
        ...
        f.write(",\n".join(base_rows))
        if fe_rows:
            # The leading comma chains the front-end rows onto the base list only
            # when the front-end build compiles them.
