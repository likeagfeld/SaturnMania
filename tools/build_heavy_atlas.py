...
def main():
    ...
    # ---- 5. Emit HBHPAL.BIN: 5 blocks x 128 colors (RGB555, the engine copies
    # straight to CRAM[block..block+127]). Each block = the first 128 GCT entries
    # of that Heavy's palette .gif. Index 0 stays the source color (transparent at
    # draw time -- pixel 0 is skipped by VDP1). ----
    palout = bytearray()
    for who, binname, palgif, anims_needed in HEAVIES:
    ...
    # S4: 5 blocks * 128 colors * 2 bytes
    assert len(palout) == 5 * 128 * 2, len(palout)
    ...
    print("HBHPAL.BIN  %6d B  (5 x 128 RGB555 blocks @ CRAM[512/768/1024/1280/1536])"
          % len(palout))
