...
OBJCLASS_SIZE = 68   # MEASURED 2026-07-21: all known _Update ptrs share (offset%68==16)
# -> the true ObjectClass stride is 68, NOT 72 (hash[16] then update@16). The wrong 72
# drifted every entry -> corrupted staticVars(@+56) reads (false-CLASSREG flood) AND the
# name lookup. RSDK layout: hash[0..15], update@16, ... staticVars@+52 (see registration()).
STAGEIDS = 0x002FEF80
...
# G1 PLACEMENT ground-truth: authored per-entity (class -> [[x_px,y_px],...]) from
# EVERY Scene.bin (tools/build_placement_manifest.py). Lets D-PLACEMENT diff live spawn
# positions vs authored FROM LIVE MEMORY, no visual comparison. Keyed by folder->scene.
try:
    PLACEMENTS = json.loads((_ROOT / "docs/scene_placements.json").read_text())
except Exception:
    PLACEMENTS = {}
# Classes bolted to the level (do NOT roam from their authored spawn) -> a live instance
# far from ALL authored positions of its class == a real placement bug. Movers (Player,
# badniks, Tails) + collectible-despawn + track-movers are EXCLUDED to avoid false flags.
PLACEMENT_STATIC = {
    # TRULY-IMMOVABLE only (2026-07-21, tightened): a class here must NEVER move from its
    # authored spawn. EXCLUDED as settle/movers after decomp check: ItemBox (ItemBox_State_
    # Falling -> boxes placed in air FALL to the ground = authored.y != live.y is CORRECT,
    # a false-positive source), plus Bumper/Fan/Whirl/conveyor-riders. Rings/Springs/StarPost/
    # SignPost/Spikes/Decoration/Pillar stay fixed at their authored coords.
    "Ring", "Spring", "StarPost", "SignPost", "Spikes", "Decoration", "Pillar",
    "SpecialRing", "GiantRing", "Spikes2", "BreakableWall",
}
...
try:
    ...
    _cur = re.search(r"0x([0-9a-fA-F]{16})\s+_end\b", MAP, re.M)  # map has '_end = .' -> \b not $
...
class Oracle:
    ...
    def registration(self):
        ...
        for listIdx, cid_idx in enumerate(ids):
            ...
            sv_ptr = int.from_bytes(cls_raw[off + 52:off + 56], "big")  # staticVars @ +52 (9 fn-ptrs 16..48, stride 68)
            ...
            # CLASSREG index-space fix (2026-07-21, parity-audit workflow INV[2], conf=high):
            # a LIVE entity's classID field == the SCENE-LOCAL list index (listIdx), per
            # decomp Scene.cpp (live classID == the stageObjectIDs index), NOT the global
            # objectClassList REGISTRATION index (cid_idx = stageObjectIDs[listIdx]). For
            # stage-local classes listIdx != cid_idx, so keying id2name by cid_idx skewed
            # D3/D4/D5 name resolution (the "class-table read-skew / staticVars invalid at
            # 51/52" false lead). Make classID = the runtime o-space key (listIdx); keep
            # regIdx for the objectClassList address (sv_ok already used cid_idx correctly
            # at :375-382). QA-tool-only -- the engine ObjectClass table is correct.
            rows.append({"listIdx": listIdx, "regIdx": cid_idx, "classID": listIdx, "name": nm, "sv_ok": sv_ok})
        return rows
    ...
    def sample(self):
        ...
        # pink-nonkey (2026-07-21): magenta at index!=0 == a DRAWN wrong colour; index-0
        # of each 256-block is RSDK's transparent KEY (Saturn keys pixel-0 invisible) ->
        # benign. nonkey==0 means the whole magenta count is transparent keys (artifact).
        s["cram_mag_nonkey"] = self.w("p6_w_cram_mag_nonkey")
        ...
        # autorun player-physics diagnostic (P6_GHZ_AUTORUN-gated -> None in shipping):
        # distinguishes an autorun HARNESS-PARK (input stopped, gvel~0, grounded, no wall)
        # from a REAL engine freeze (death/no-respawn, OR gvel high vs a collision wall).
        # Contract p6_io_main.cpp:9706 -- "~0 gvel == input stopped, high gvel == wall".
        s["pdiag_posx"] = s32(self.w("p6_w_pdiag_posx"))
...
def dump_classmap():
    ...
    for cid in range(128):
        ...
        if nm:
            out[cid] = nm
    fp = re.search(r"0x([0-9a-fA-F]{16})\s+_end\b", MAP, re.M)  # map has '_end = .' -> \b not $
...
def main():
    ...
    for folder, ss in scenes.items():
        ...
        if folder not in UI_SCENES and expected:
            ...
            # PLACEMENT-GATED (2026-07-21): only flag "placed-but-not-spawned" when the
            # class is ACTUALLY placed in this folder's Scene.bin (docs/scene_placements.
            # json). A registered-but-unplaced global (e.g. Splats in GHZ = 0 placements)
            # having 0 live instances is CORRECT, not a bug -- was a SPAWN false-positive.
            _place = PLACEMENTS.get(folder, {})
            _placed_pts = {}   # class -> [(x,y),...] across all scene files of this folder
            for _sc in _place.values():
                for _c, _pts in _sc.items():
                    _placed_pts.setdefault(_c, []).extend(_pts)
            _placed_ct = {c: len(v) for c, v in _placed_pts.items()}
            # CAMERA-WINDOW GATE (2026-07-21): the Saturn pool is CAMERA-LOCAL -- it only
            # materializes entities NEAR the camera. A placed class whose placements are ALL
            # far from the live camera legitimately has 0 live instances (not yet streamed) --
            # flagging it is the same artifact class as off-camera ANIM. MEASURED: SpikeLog
            # (placed x2632-4552) / SpinBooster (x5800-6116) both 0-live because the camera
            # sat at x8238 -- thousands of px past them. Only flag "not spawned" when the
            # class has a placement WITHIN STREAM_WIN of the camera but is still 0 live.
            _cam = next((e for e in last["entities"] if e["classID"] == 6), None) \
                or next((e for e in last["entities"] if e["classID"] == 8), None)
            _camx = _cam["x"] if _cam and _cam.get("x") is not None else None
            STREAM_WIN = 700  # px each side (Rings/Springs materialize within ~400; margin)
            if (last["n_entities"] or 0) >= 4:
                for nm in sorted(expected & reg_names):
                    if nm in NONSPAWN:
                        continue
                    if PLACEMENTS and _placed_ct.get(nm, 0) == 0:
                        continue   # registered but NOT placed in this scene -> 0 live is correct
                    # camera-window gate: need a placement near the live camera to judge
                    if PLACEMENTS and _camx is not None and nm in _placed_pts:
                        _near = any(abs(px - _camx) <= STREAM_WIN for px, py in _placed_pts[nm])
                        if not _near:
                            continue   # all placements off-camera -> not-yet-streamed, can't judge
        ...
        # D16 PLACEMENT (G1, 2026-07-21): a live STATIC-class entity must sit at one of
        # its class's AUTHORED Scene.bin positions (docs/scene_placements.json). If a live
        # instance is far from ALL authored positions of its class, it spawned at the wrong
        # coordinates -- a placement bug, caught FROM LIVE MEMORY, no visual comparison.
        # Restricted to PLACEMENT_STATIC (level-bolted objects) so roaming badniks / the
        # player / collectible-despawn / track-movers don't false-flag. Union all scene
        # files for the folder (the runtime may be on act 1 or 2). TOL is generous (24px)
        # to absorb Q16.16 rounding + the Saturn x1 entity-scale retarget.
        place = PLACEMENTS.get(folder, {})
        if place and last["entities"]:
            authored = {}   # class -> list of (x,y)
            for scdict in place.values():
                for cls, pts in scdict.items():
                    if cls in PLACEMENT_STATIC:
                        authored.setdefault(cls, []).extend(tuple(p) for p in pts)
            PLACE_TOL = 24
            reported = 0
            for e in last["entities"]:
                nm = id2name.get(e["classID"])
                if nm not in authored or e["x"] is None or e["y"] is None:
                    continue
                pts = authored[nm]
                nearest = min((abs(e["x"] - ax) + abs(e["y"] - ay)) for ax, ay in pts)
                if nearest > PLACE_TOL and reported < 6:   # cap noise; 6 exemplars/scene
                    ax, ay = min(pts, key=lambda p: abs(e["x"] - p[0]) + abs(e["y"] - p[1]))
                    D(folder, "PLACEMENT", f"'{nm}' slot{e['slot']} live@({e['x']},{e['y']}) is "
                                           f"{nearest}px (manhattan) from its nearest AUTHORED pos "
                                           f"({ax},{ay}) -- spawned at wrong coordinates")
                    reported += 1

        # D4 ANIM: any DRAWABLE live entity with an out-of-range animID for its class.
        # DRAWABLE GATE (2026-07-21, live-validated -- was a D4 semantic artifact class):
        # an out-of-range animID only means "wrong POSE on screen" if the entity is
        # actually being drawn. In the camera-local streaming pool, entities FAR from
        # the camera are not yet activated -- their Animator is uninitialized memory, so
        # animID reads garbage BY DESIGN (measured: cls51 splits {0, 19228} across
        # instances = per-instance uninit, not a wrong-offset read; cls28 all visible==0;
        # ALL garbage-animID entities lay OUTSIDE the camera box [cam.x+/-288, cam.y+/-240]
        # while only the 2 on-screen entities read sane animIDs). Saturn's engine-set
        # onScreen (@+86) is UNRELIABLE (only 2/210 set it live), so gate on the
        # game-logic `visible` flag AND camera proximity, not onScreen. Viewport center =
        # the camera entity (classID 6) if present, else the player (classID 8).
        cam = next((e for e in last["entities"] if e["classID"] == 6), None) \
            or next((e for e in last["entities"] if e["classID"] == 8), None)
        DRAW_MARGIN_X, DRAW_MARGIN_Y = 288, 240   # half-screen(160/112) + max sprite extent
        for e in last["entities"]:
            ...
            if aid is None:
                continue
            # drawable gate: must be visible AND within the drawable viewport
            if not (e.get("visible") or 0):
                continue
            if cam is not None and e["x"] is not None and e["y"] is not None \
               and cam["x"] is not None and cam["y"] is not None:
                if abs(e["x"] - cam["x"]) > DRAW_MARGIN_X or abs(e["y"] - cam["y"]) > DRAW_MARGIN_Y:
                    continue   # off-camera -> un-activated streamer, animator uninit by design
            elif cam is None:
                # UI-SCENE FALLBACK (2026-07-21): Logos/Title/Menu have no camera(cls6)
                # AND no player(cls8) -> we CANNOT bound the drawable viewport, so a
                # garbage-animID can't be confirmed on-screen. UI widgets (UIPicture etc.)
                # also don't use the standard Animator layout the +112 offset assumes ->
                # high false-positive rate. Downgrade to a NOTE (report, don't count) so a
                # real UI wrong-pose is still visible without flooding the divergence list.
                if aid >= 0xFF00 or aid > 200:
                    print(f"  [{folder or '?':12s}] NOTE     entity '{nm or ('cid'+str(e['classID']))}' "
                          f"slot{e['slot']} animID={aid} unverifiable (UI scene, no camera to bound "
                          f"drawable; UI widget Animator layout differs) -- not counted")
                continue
            if aid >= 0xFF00 or aid > 200:
        ...
        for k, v in last_e.items():
            ...
            if d > 0:
                edge_delta[k] = d
        # BENIGN-ORDINAL WHITELIST (2026-07-21, classified via p6_closure_edge.c; NARROWED
        # after the user flagged menu glitches). ONLY genuinely-dead-on-Saturn stubs that
        # cannot affect parity IN ANY SCENE are whitelisted:
        #   47 APICallback_GetUserAuthStatus    -- online/achievements; N/A on Saturn
        #   48 APICallback_InputIDIsDisconnected-- input hotplug; Saturn pads are fixed
        # DELIBERATELY NOT whitelisted (they are MENU-LOAD-BEARING, not benign):
        #   3  CompetitionSession_GetSession -- backs the Competition-Mode menu row; the
        #      stub returned NULL and MenuSetup dereffed it UNGUARDED (MenuSetup:824
        #      `session->inMatch` + 6 more) -> low-mem garbage -> menu mode-select GLITCH.
        #      FIXED (p6_closure_edge.c: zero-init instance). Kept UN-whitelisted so any
        #      future menu regression here still fires -- a whitelist would HIDE it.
        #   20 MenuParam_GetParam -- menu navigation/selection state; same NULL-deref class.
        # 3(CompetitionSession)/20(MenuParam) are NOW FIXED to return valid zero-init
        # instances (p6_closure_edge.c) -> crossing them is benign; the EDGE crossing
        # count never detected the NULL-deref anyway (that's fixed at SOURCE). 4=
        # CompetitionSession_ResetOptions, 65=APICallback_TrackEnemyDefeat (achievement)
        # are single-player no-ops. All benign -> whitelist.
        BENIGN_EDGE = {3, 4, 20, 47, 48, 65}
        benign_hit = {k: v for k, v in edge_delta.items() if k in BENIGN_EDGE}
        real_edge = {k: v for k, v in edge_delta.items() if k not in BENIGN_EDGE}
        if benign_hit:
            print(f"  [{folder or '?':12s}] NOTE     edge stubs crossed but DEAD-COSMETIC (single-player "
                  f"no-ops) {dict(sorted(benign_hit.items()))} -- CompetitionSession/MenuParam/UserAuth/"
                  f"InputHotplug; correct single-player behaviour, not a divergence")
        if real_edge:
            D(folder, "EDGE", f"edge-audit ordinals fired IN this scene {dict(sorted(real_edge.items()))} "
        ...
        if folder in AUDIO_EXPECT:
            ...
            # PARK-ROBUST BGM (2026-07-21, live-validated): the CD-DA legitimately ENDS
            # after its track length; a MULTI-MINUTE-PARKED sample then reads str_track=-1
            # + stream_frames FLAT even though the BGM played correctly on arrival. MEASURED
            # at fresh GHZ: str_track=2, stream_frames INCREMENTING (972->1028) = playing.
            # So judge by whether the CORRECT track was EVER armed OR the stream EVER
            # advanced DURING the window -- not the parked instant. Only a track NEVER
            # correctly armed (or a genuinely WRONG track) is a real BGM bug.
            _strf_all = [s.get("stream_frames") for s in ss if s.get("stream_frames") is not None]
            stream_grew = len(_strf_all) >= 2 and (max(_strf_all) - min(_strf_all)) > 0
            track_ok = bool(exp) and any((t in exp) for t in tracks)
            bgm_played = track_ok or stream_grew
            if tracks and max(tracks) <= 0 and not bgm_played:
            ...
            elif not stream_on and not bgm_played:
                ...
                print(f"  [{folder or '?':12s}] NOTE     BGM OK: correct track {sorted(t for t in tracks if t>0)} "
                      f"armed and stream {'advanced' if stream_grew else 'live'} during the window "
                      f"(a later flat/-1 reading is the CD track ending during a long park, not a bug)")
            if exp is not None and tracks:
                # WRONG music only if a DIFFERENT valid track played AND the correct one never did
                wrong = sorted(t for t in tracks if t > 0 and t not in exp)
                if wrong and not track_ok:
            ...
            # PARK-ROBUST: flat stream_frames is only a bug if the correct track was ALSO
            # never armed (bgm_played gates it -- see above). A flat window with the right
            # track already armed = the CD track ended during a long park, not silence.
            if strf and (max(strf) - min(strf)) == 0 and folder in BGM_EXPECT and not bgm_played:
        ...
        if len(mags) >= 2 and (max(mags) - min(mags)) >= FLASH_SWING:
            ...
            # NONKEY GATE (2026-07-21): only DRAWN magenta (index!=0) is a real wrong
            # colour. If every magenta entry is a per-block index-0 transparent KEY
            # (nonkey==0), the "composed region" count is benign -> NOTE, not divergence.
            nonkeys = [s.get("cram_mag_nonkey") for s in ss if s.get("cram_mag_nonkey") is not None]
            max_nonkey = max(nonkeys) if nonkeys else None
            # COMP-VARIANCE GATE (2026-07-21, live per-bank proof): banks-0-2 magenta
            # STATIC across the window is the UNWRITTEN palette-block-tail baseline, NOT
            # drawn pink -- the SAME "displayed palettes vary, unwritten CRAM is constant"
            # principle that established banks 3-7 as artifact. Measured live at settled
            # GHZ: banks0-2=[1,38,187] CONSTANT, i.e. bank2 is 187/256 magenta -- a whole-
            # block unwritten tail (a real per-sprite wrong colour touches a handful of
            # REFERENCED indices, and 73%-of-a-block magenta would render GHZ as a solid
            # magenta blob, which no visual capture shows). A real COMPOSED-region pink is
            # a CHANGE, so flag on WITHIN-SCENE comp SWING (like FLASH_SWING for total),
            # not static presence. CRAM alone cannot separate a STATIC referenced wrong
            # colour from a STATIC unwritten tail (needs per-index VDP1 pixel refs) -> a
            # static comp is a NOTE naming that limit, never a counted divergence.
            comps = [sum(s["cram_mag_bank"][:3]) for s in ss
                     if s.get("cram_mag_bank") and len(s["cram_mag_bank"]) >= 3]
            comp_swing = (max(comps) - min(comps)) if len(comps) >= 2 else 0
            if comp is not None and comp_swing >= FLASH_SWING and (max_nonkey is None or max_nonkey > 0):
                D(folder, "PALETTE", f"COMPOSED-region pink FLASH -- banks0-2 magenta swung "
                                     f"{min(comps)}->{max(comps)} WITHIN this scene (drawn-nonkey={max_nonkey}) "
                                     f"-- a real composition change in a displayed NBG/palette")
            elif comp is not None and comp > 24 and max_nonkey == 0:
                print(f"  [{folder or '?':12s}] NOTE     magenta banks0-2={banks[:3]} but drawn-nonkey=0 "
                      f"-- ALL transparent index-0 keys (invisible on Saturn), NOT a visible colour bug")
            elif comp is not None and comp > 24:
                print(f"  [{folder or '?':12s}] NOTE     banks0-2 magenta STATIC at {banks[:3]} "
                      f"(swing {comp_swing} < {FLASH_SWING}; nonkey={max_nonkey}) -- an UNWRITTEN "
                      f"palette-block tail (bank fill fraction, not scattered refs), NOT visible pink; "
                      f"CRAM alone can't confirm a static referenced wrong-colour (needs VDP1 pixel refs)")
            else:
        ...
        # D9-TEMPORAL BLIND SPOT (2026-07-21, user SEES pink flashes the CRAM-count gate MISSED):
        # a pink FLASH is a sub-second DISPLAY/COMPOSITION transient (a magenta-key transparent
        # region rendering OPAQUE for 1-2 frames when a VDP2 sky/BG plane arm or priority hiccups
        # -- SGL slScrAutoDisp B1-drop gotcha #7). The magenta is ALREADY in CRAM (keys+tails) so
        # the COUNT never changes -> the D9 count/swing gate above is STRUCTURALLY blind, AND the
        # 1-2 Hz oracle sampling is Nyquist-blind to a sub-second flash. This is NOT closed.
        # LOUD anti-lying NOTE so pink is never read as GREEN until a per-frame (60Hz, SH-2-side)
        # pink-VISIBLE witness (BGON plane-drop + VDP1-fb magenta-pixel, accumulated monotonic)
        # is added. Scenes with a magenta-key sky: GHZ/AIZ/GHZCutscene/Title.
        if folder in ("GHZ", "AIZ", "GHZCutscene", "Title"):
            print(f"  [{folder or '?':12s}] NOTE     PINK-FLASH temporal blind spot: a sub-second "
                  f"display-composition pink flash (plane-drop exposing the magenta-key sky) is NOT "
                  f"instrumented -- D9 measures CRAM COUNT (static) + samples at 1-2Hz, both blind to "
                  f"it. OPEN (user-reported). Needs a per-frame pink-VISIBLE witness; do NOT read "
                  f"PALETTE as GREEN here.")
        ...
        if folder not in UI_SCENES and len(ss) >= 4:
            ...
            if len(good) >= 4 and len(set(good)) == 1:
                # AUTORUN HARNESS-PARK vs REAL FREEZE (2026-07-21, live-validated). A
                # frozen player X is NOT a divergence when the engine is still ticking
                # AND the player is grounded with ~0 ground velocity and no collision
                # wall -- that is the autorun input simply stopping at a nav gap (memory
                # ghz-autorun-stall: harness gaps, collision verified, NOT engine bugs;
                # contract p6_io_main.cpp:9706). LIVE-MEASURED at GHZ x8246: gvel=0,
                # gnd=1, plane/mode/ang=0 -> park. Downgrade to an anti-lying NOTE so it
                # is recorded, not silently dropped, and does NOT contaminate the count.
                # A REAL freeze (death/no-respawn, or gvel high vs a wall) still fires.
                ticks = [s.get("tick") for s in ss if s.get("tick") is not None]
                tick_live = len(ticks) >= 2 and max(ticks) > min(ticks)
                gvel = last.get("pdiag_gvel"); gnd = last.get("pdiag_gnd")
                park = (tick_live and gnd == 1 and gvel is not None
                        and abs(gvel) <= 0x4000)  # <0.25 px/frame ground vel = input off
                if park:
                    print(f"  [{folder or '?':12s}] NOTE     player X parked at {good[0]} "
                          f"(gvel~0, grounded, engine ticking) -- AUTORUN nav-gap, not an "
                          f"engine freeze (harness limitation; a human holding RIGHT would "
                          f"progress). Boot->signpost traversal blocked by the QA harness, "
                          f"NOT a parity bug.")
                else:
                    D(folder, "TEMPORAL", f"player X frozen at {good[0]} across {len(good)} "
                                          f"samples (gvel={gvel}, gnd={gnd}, tick_live={tick_live}) "
                                          f"-- stuck-state (death/no-respawn, or gvel-vs-wall)")
    ...
    # D15 SEAM/LOAD + PREBOOT (2026-07-21, user-mandated -- load-time IS parity, the PC
    # loads each scene near-instantly while the Saturn is CD-seek-bound 6-13s). The oracle
    # previously WAITED PAST load windows (settle) without timing them. Measure per-scene
    # LOAD-TO-SETTLE = (first SETTLED sample of a folder) - (first sample where that folder
    # became active): currentSceneFolder is set at LoadSceneFolder START, entities populate
    # at load END, so that delta IS the scene's asset-load time. PREBOOT = wall of the first
    # scene's first sample (valid ONLY if the oracle was started at RA launch; else it reads
    # ~0 and is reported as a NOTE with that caveat). Coarse to PERIOD sampling, which is
    # fine -- it catches the multi-second stalls that matter. Budgets (memory
    # load-seams-cd-seek-bound-not-boot): AIZ ~13s / GHZ ~6.1s CD-seek-bound; PC ~1-2s.
    SEAM_BUDGET = 9.0
    first_seen, first_settled, order = {}, {}, []
    for s in samples:
        f = s.get("folder")
        if not f:
            continue
        if f not in first_seen:
            first_seen[f] = s["wall"]; order.append(f)
        n = s.get("n_entities") or 0
        settled_now = n > 3 or (f in UI_SCENES and n >= 1)
        if settled_now and f not in first_settled:
            first_settled[f] = s["wall"]
    loads = {f: round(first_settled[f] - first_seen[f], 1) for f in order if f in first_settled}
    if loads:
        print(f"  [{'(load)':12s}] NOTE     per-scene load-to-settle (s): {loads}")
        for f, d in loads.items():
            if d > SEAM_BUDGET:
                D(f, "SEAM", f"{f} scene took {d:.1f}s to load+settle (> {SEAM_BUDGET:.0f}s budget) "
                             f"-- CD-seek-bound load-time parity miss vs PC ~1-2s (#322 load campaign)")
    if order:
        preboot = round(first_seen[order[0]], 1)
        if preboot >= 12.0:
            D(order[0], "PREBOOT", f"first scene ({order[0]}) not sampled until t={preboot}s "
                                   f"-- boot/preboot cost (valid iff oracle started at RA launch)")
        else:
            print(f"  [{'(preboot)':12s}] NOTE     first scene {order[0]!r} at t={preboot}s "
                  f"(preboot cost; only meaningful if oracle started at RA launch)")
