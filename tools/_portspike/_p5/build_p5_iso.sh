#!/usr/bin/env bash
# Build the minimal P5 first-object-proof ISO (Task #204). Stages ONLY the P5
# first-read payload 0.bin + the three IP-referenced PVD text files (ABS/CPY/BIB),
# then runs the SAME mkisofs recipe the jo build uses (jo_engine_makefile:310-312):
# -generic-boot IP.BIN embeds the jo COMMON IP.BIN (load/exec 0x06004000, the
# p5.linker .text base) at LBA 0; 0.bin sorts first in the root dir (ASCII '0' <
# letters) so the Saturn first-read mechanism loads it to 0x06004000 and jumps.
# Emits _p5.iso (a 2048-byte/sector image despite -sectype 2352 -- the IP header
