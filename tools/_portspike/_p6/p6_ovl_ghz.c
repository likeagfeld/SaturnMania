...
/* PAN-TARGET (#322) NOTE: the minimal p6_aizplat_* Platform stubs are RETIRED --
 * Batch 3 step 2 registers the FULL verbatim Common/Platform.c (Game_Platform.o,
 * overlay-resident, ALL flavors) below. AIZSetup's foreach_all(Platform) +
 * the beat-4 lerp target (AIZSetup.c:279-286/457) bind intra-overlay to the same
 * Serialize var list, so qa_aiz_pan_target.py's decomp-exact 11056 target holds;
 * the dig-site platforms additionally gain their real Create/Draw (PLATFORM_FIXED
 * + AIZ/Platform.bin via AIZOBJ.PAK -> the wooden platforms render, PC parity). */
#endif
...
int p6_overlay_entry(p6_ovl_api *api)
{
    ...
    /* BATCH 3 step 2 (2026-07-09): the FULL verbatim Common/Platform.c
     * (Game_Platform.o, overlay-resident, registered in ALL flavors -- replaces
     * the #322 minimal P6_AIZ_TEST-only stub). GHZ1 authors 59 Platform entities
     * (Scene1.bin parse: types 0 FIXED / 1 FALL / 2 LINEAR / 4 SWING / 6 PUSH;
     * childCount chains at slots 793/815). Entity 204 B <= 344 narrow stride.
     * Closure (line-cited): Player_CheckCollision{Box,Platform,Touch} /
     * _GetHitbox / _Hurt / _State_{Air,Static,KnuxWallClimb,KnuxLedgePullUp} /
     * _Action_Jump + Zone_RotateOnPivot (pack, -u rooted); Ice (pack global,
     * :80 NULL-guarded); ItemBox/Spikes (:39/:59, intra-overlay, registered);
     * HangPoint (:363 NULL-guarded) + PlatformNode/PlatformControl (:1064/:1180,
     * State_Path* only -- no GHZ1 PATH/TRACK types) = pack NULL placeholders.
     * Every Player_CheckMightyUnspin / State_MightyHammerDrop ref is
     * MANIA_USE_PLUS-gated (verified per-site) -> drops at PLUS=0.
     * Anims: GHZ/Platform.bin (cart GHZOBJ.PAK), AIZ/Platform.bin (AIZOBJ.PAK),
     * GHZCutscene/Platform.bin (HBHOBJ.PAK); other folders' loads miss their
     * pack -> aniFrames=-1 -> DrawRect placeholder ONLY where entities exist
     * (none authored outside GHZ/AIZ/GHZCutscene in the ported scene set). */
