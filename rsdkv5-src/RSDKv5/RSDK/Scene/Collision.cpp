                ...
                for (int32 i = 0; i < 3; ++i) {
                    if (cy >= 0 && cy < TILE_SIZE * layer->ysize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            int32 ty        = cy + mask;
                            int32 tileAngle = RSDK_FLOOR_ANGLE(collisionEntity->collisionPlane, tile);
                ...
                for (int32 i = 0; i < 3; ++i) {
                    if (cx >= 0 && cx < TILE_SIZE * layer->xsize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            int32 tx        = cx + mask;
                            int32 tileAngle = RSDK_LWALL_ANGLE(collisionEntity->collisionPlane, tile);
                ...
                for (int32 i = 0; i < 3; ++i) {
                    if (cy >= 0 && cy < TILE_SIZE * layer->ysize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            int32 ty        = cy + mask;
                            int32 tileAngle = RSDK_ROOF_ANGLE(collisionEntity->collisionPlane, tile);
                ...
                for (int32 i = 0; i < 3; ++i) {
                    if (cx >= 0 && cx < TILE_SIZE * layer->xsize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            int32 tx        = cx + mask;
                            int32 tileAngle = RSDK_RWALL_ANGLE(collisionEntity->collisionPlane, tile);
                    ...
                    if (cy >= 0 && cy < TILE_SIZE * layer->ysize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            if (mask < 0xFF) {
                                ...
                                step = -TILE_SIZE;
                                if (colY < collidePos) {
                                    collideAngle = RSDK_FLOOR_ANGLE(collisionEntity->collisionPlane, tile);
                                    collidePos   = ty;
                                    i            = stepCount;
#else
                                if (colY >= ty) {
                                    if (abs(colY - ty) <= collisionMinimumDistance) {
                                        sensor->collided   = true;
                                        sensor->angle      = RSDK_FLOOR_ANGLE(collisionEntity->collisionPlane, tile);
                                        sensor->position.y = TO_FIXED(ty + layer->position.y);
                                        i                  = stepCount;
                                    }
#endif
                    ...
                    if (cx >= 0 && cx < TILE_SIZE * layer->xsize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            if (mask < 0xFF && colX >= tx && abs(colX - tx) <= 14) {
                                sensor->collided   = true;
                                sensor->angle      = RSDK_LWALL_ANGLE(collisionEntity->collisionPlane, tile);
                                sensor->position.x = TO_FIXED(tx + layer->position.x);
                                i                  = 3;
                            }
                        }
                    ...
                    if (cy >= 0 && cy < TILE_SIZE * layer->ysize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            if (mask < 0xFF) {
                                ...
                                step = TILE_SIZE;
                                if (colY > collidePos) {
                                    collideAngle = RSDK_ROOF_ANGLE(collisionEntity->collisionPlane, tile);
                                    collidePos   = ty;
                                    i            = stepCount;
#else
                                if (colY < ty) {
                                    if (abs(colY - ty) <= collisionMinimumDistance) {
                                        sensor->collided   = true;
                                        sensor->angle      = RSDK_ROOF_ANGLE(collisionEntity->collisionPlane, tile);
                                        sensor->position.y = TO_FIXED(ty + layer->position.y);
                                        i                  = stepCount;
                                    }
#endif
                ...
                for (int32 i = 0; i < 3; ++i) {
                    if (cx >= 0 && cx < TILE_SIZE * layer->xsize) {
                        ...
                        if (tile < 0xFFFF && tile & solid) {
                            ...
                            if (mask < 0xFF && colX <= tx && abs(colX - tx) <= 14) {
                                sensor->collided   = true;
                                sensor->angle      = RSDK_RWALL_ANGLE(collisionEntity->collisionPlane, tile);
                                sensor->position.x = TO_FIXED(tx + layer->position.x);
                                i                  = 3;
                            }
                        }
